...
function parsePostgresDsn(dsn: string): DsnParts & {
  ...
  try {
    ...
    const db = decodeURIComponent(
      (u.pathname.replace(/^\//, '').split('/')[0] || 'bitmagnet').trim(),
    );
    return {
      host: u.hostname || '',
      port: u.port || '5432',
...
function buildPostgresDsn(parts: DsnParts & { password: string }): string {
  ...
  const database = (parts.database.trim() || 'bitmagnet').replace(/^\/+/, '');
...
export function BitmagnetDbPanel({
  ...
  const [database, setDatabase] = useState('bitmagnet');
    ...
    void (async () => {
      try {
        ...
        if (cfg.enabled && cfg.dsn) {
          setConnected(null);
          try {
            const r = await testBitmagnetDb(cfg.dsn);
            if (cancelled) return;
            setConnected(r.ok);
  ...
  async function onTest() {
    ...
    setMsg('');
    try {
      const r = await testBitmagnetDb(resolveDsn());
      setConnected(r.ok);
  ...
  async function onSave() {
    ...
    try {
      ...
      await putBitmagnetDb({ enabled, dsn, note });
      const savedPwd =
      ...
      if (enabled && dsn) {
        try {
          const r = await testBitmagnetDb(dsn);
          setConnected(r.ok);
          onStatus(r.ok ? '已连接' : '连接失败', r.ok ? 'ok' : 'warn');
  ...
  return (
    <AppPush title="Bitmagnet 库" onBack={onBack}>
      ...
      {isAdmin ? (
        ...
        <AppFootnote>普通用户仅可查看连接状态。Bitmagnet 与色花资源库分开配置。</AppFootnote>
      )}
    </AppPush>
  );
}
