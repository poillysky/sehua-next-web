...
import app.core.pg as pg
from app.core.pg import ResourceDbUnavailable
from app.search.resource_format import PUBLIC_RESOURCE_FILTER
from app.search.av import (
    code_sort_key,
...
def _append_cover_fallbacks(
    ...
    from app.search.pack_bleed import image_url_matches_maker_code, is_jacket_cover_url
...
def _prefix_tile_covers(
    ...
    from app.search.pack_bleed import image_url_matches_maker_code, pick_covers_for_code
...
def _is_domestic_or_clear_uncensored_row(row: dict[str, Any]) -> bool:
    ...
    from app.core.region_board_allowlist import board_fid_allowed
...
def _post_zh_keep_score(desc: str, title: str, *, code: str = "") -> int:
    ...
    from app.search.pack_bleed import get_description_field
    from app.scrape.forum_title import is_fake_forum_title_fast
...
def _rows_to_prefix_index(
    ...
    *,
    ...
    from app.search.forum_seed import pick_forum_seed_from_posts
    from app.search.pack_bleed import get_description_field
    from app.core.region_board_allowlist import board_fid_allowed, normalize_region

    ordered = _sort_for_prefix_cover(rows)
    region_key = normalize_region(region)
    from app.core.region_meta import indexes_forum_actors
    from app.search.pack_bleed import own_maker_codes_for_index_row
...
def _collect_posts_by_code_from_rows(
    ...
    from app.search.pack_bleed import get_description_field, own_maker_codes_for_index_row
...
def _apply_zh_title_boost(
    ...
    from app.search.forum_seed import pick_forum_seed_from_posts
    from app.core.region_meta import indexes_forum_actors
    from app.scrape.forum_title import is_fake_forum_title, is_likely_chinese
    ...
    for it in need:
        ...
        from app.search.forum_seed import (
...
def _scan_likes_via_hash_cte(
    ...
    *,
    ...
    from app.core.region_board_allowlist import normalize_region
...
def _std_code_key(code: str, *, pad: int = 3) -> str:
    ...
    from app.search.av import std_code_key
