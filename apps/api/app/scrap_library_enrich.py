...
from .scrap_library_nfo import parse_nfo, write_nfo
...
def _clean_actors(names: list[str] | None) -> list[str]:
    from .scrape_metadata_optimize import _clean_actor_raw
...
def _fold_tag_variant(tag: str) -> str:
    ...
    try:
        from .scrape_metadata_optimize import _fold_variant
...
def _canonical_enrich_log_region(region: str | None = None) -> str:
    ...
    if not raw:
        return "_all"
    from .region_meta import REGION_META
...
def get_enrich_status() -> dict[str, Any]:
    ...
    try:
        from .region_meta import REGION_ORDER
...
def _normalize_tag_alias(tag: str) -> str:
    ...
    if not t:
        return ""
    try:
        from .scrape_metadata_optimize import polish_tag_names
...
def _strategy_field_priority(field: str) -> list[str] | None:
    try:
        from .scrap_enrich_strategy import get_strategy
    ...
    if len(_clusters) >= 2:
        ...
        if (
            ...
            try:
                ...
                from .scrape_metadata_optimize import _fold_variant as _fold  # noqa: F401
            _acts_by_sid = {
                _sid: {
    ...
    if tag_lists:
        ...
        for i, (_sid, tags, _sc) in enumerate(tag_lists):
            ...
            if len(tags_out) >= 24:
                break
        try:
            from .scrape_metadata_optimize import code_prefix, polish_tag_names
...
def _maybe_llm_fill_zh(detail: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    try:
        from .translate_routes import translate_to_zh_sync
    except Exception as e:  # noqa: BLE001
