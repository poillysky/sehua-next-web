...
def upsert_actress_profile(
    ...
    *,
    ...
    if not rel:
        try:
            from .avatar import resolve_avatar_rel
...
def actress_ages_for_names(names: list[str]) -> dict[str, int]:
    ...
    """
    from .bio import age_from_birthday, lookup_actress_db
    ...
    try:
        from .aliases import alias_names as _alias_names
    except Exception:  # noqa: BLE001
...
def sync_actresses_to_vector_db(
    ...
    *,
    ...
    from .avatar import ensure_actress_avatar
    from app.scrape.metadata_optimize import polish_actress_names
    ...
    if ensure_avatar:
        try:
            from .avatar import _load_gfriends_index
